...
import com.cops.scada.entity.Site;
import com.cops.scada.service.UploadService;
import com.cops.scada.util.QiniuFileUtil;
import com.cops.scada.util.RestResponse;
import com.cops.scada.util.ToolUtil;
...
public class FileController {
    ...
    public RestResponse test() {
        return RestResponse.success().setMessage("test");
    }
    ...
    public RestResponse downloadFile(@RequestParam(value="url",required = false) String realurl,
                                 @RequestParam(value="name",required = false) String name,
                                 HttpServletResponse response) throws IOException {
        if(StringUtils.isBlank(realurl)){
            return RestResponse.failure("文件地址不能为空");
        }
        if(StringUtils.isBlank(name)){
        ...
        name = new String(name.getBytes("GB2312"),"ISO8859-1");
        FileInputStream fis = new FileInputStream(localUploadPath+realurl);
        ...
        response.setHeader("Content-Disposition",
                "attachment; filename="+name + realurl.substring(realurl.lastIndexOf(".")));
        ServletOutputStream out = response.getOutputStream();
