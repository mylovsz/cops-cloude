...
<fieldset class="layui-elem-field">
  <legend>车间站点信息</legend>
  <div class="layui-field-box">
      <div class="layui-inline">
          <a class="layui-btn layui-btn-normal" data-type="addFactorySite">添加车间站点信息</a>
      </div>
  </div>
</fieldset>
...
<script type="text/javascript">
    layui.use(['form', 'tree', 'layer','table'], function() {
        ...
        var layout = [
            { name: '站点名称', treeNodes: true, headerClass: 'value_col_one', style: 'text-align: left;'},
            ...
            },
            {
                ...
                render: function(row) {
                    return '<a class="layui-btn layui-btn-normal layui-btn-sm" onclick="addChildMenu(' + row.id + ')"><i class="layui-icon">&#xe654;</i> 添加子站点</a>' +
                        '<a class="layui-btn layui-btn-normal layui-btn-sm" onclick="editChildMenu(' + row.id + ')"><i class="layui-icon">&#xe642;</i> 编辑站点</a>' +
        ...
        $(function(){
            $.post("${base}/factorySite/list",function(res){
                if(res.success){
        ...
        var active={
            addFactorySite : function(){
                var addIndex = layer.open({
                    title : "添加系统菜单",
                    type : 2,
                    content : "${base}/factorySite/add",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回站点列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
                            });
                        },500);
                    }
    ...
    var addChildMenu = function(data){
        var addIndex = layer.open({
            title : "添加站点信息",
            type : 2,
            content : "${base}/factorySite/add?parentId="+data,
            success : function(layero, addIndex){
                setTimeout(function(){
                    layer.tips('点击此处返回站点列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    var editChildMenu = function(data){
        var editIndex = layer.open({
            title : "编辑站点信息",
            type : 2,
            content : "${base}/factorySite/edit?id="+data,
    ...
    var delMenu =function(data){
        layer.confirm("你确定要删除该站点么？这将会使得其下的所有子站点都删除",{btn:['是的,我确定','我再想想']},
            function(){
                $.post("${base}/factorySite/delete",{"id":data},function (res){
                    if(res.success){
