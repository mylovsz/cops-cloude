            ...
            <div class="layui-field-box">
                <form class="layui-form" id="searchForm">
                    ...
                    <div class="layui-inline" style="margin-left: 15px">
                        <label>是否加入到筛选:</label>
                        <div class="layui-input-inline">
                            <select name="s_isSearch" lay-search>
                    ...
                    <div class="layui-inline">
                        <a class="layui-btn layui-btn-normal" data-type="addSeriesAttr">添加属性</a>
                    </div>
                </form>
            </div>
        </fieldset>
...
<script>
    layui.use(['layer','form','table','laydate'], function() {
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑属性",
                    type : 2,
                    content : "${base}/seriesAttr/edit?id="+data.id,
                    success : function(layero, index){
                        setTimeout(function(){
                            layer.tips('点击此处返回属性列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
                            });
                        },500);
                    }
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该属性么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/seriesAttr/delete",{"id":data.id},function (res){
                                if(res.success){
        ...
        var t = {
            elem: '#test',
            url:'${base}/seriesAttr/list',
            where: {s_type: seriesTypeValue},
            method:'post',
            ...
            cols: [[
                ...
                {field:'valueType', title: '值类型',templet:'#valueType'},
                {field:'isSearch', title: '是否加入到筛选',templet:'#isSearch'},
                //{field:'delFlag',    title: '属性状态',width:'12%',templet:'#userStatus'},
        ...
        var active={
            addSeriesAttr : function(){
                var addIndex = layer.open({
                    title : "添加属性",
                    type : 2,
                    content : "${base}/seriesAttr/add?typeValue="+seriesTypeValue,
