...
<#macro bpTree children>
    <#if children?? && children?size gt 0>
        <#list children as child>
            ...
            <li style="margin-top: 5px;margin-left: 1.5em"><input type="checkbox" title="${child.name}" value="${child.id}" data-parentIds = "${child.parentIds}"  lay-filter="roleMenu" <#if menuIds?split(',')?seq_contains(child.id?string)>checked</#if> />
                <ul>
                    <@bpTree children=child.children />
                </ul>
            </li>
            <#else>
            <li style="margin-top: 5px;margin-left: 1.5em"><input type="checkbox" title="${child.name}" value="${child.id}" data-parentIds = "${child.parentIds}"  lay-filter="roleMenu"  <#if menuIds?split(',')?seq_contains(child.id?string)>checked</#if> /></li>
            </#if>
        </#list>
    </#if>
</#macro>
            ...
            <#list menuList as menu>
                <div class="layui-inline">
                    <fieldset class="layui-elem-field">
                        <legend><input type="checkbox"  value="${menu.id}" data-parentIds = "${menu.parentIds}" title="${menu.name}" lay-filter="roleMenu" <#if menuIds?split(',')?seq_contains(menu.id?string)>checked</#if> /></legend>
