...
<form class="layui-form" style="width:80%;">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            ...
            <input type="text" class="layui-input" name="shortName" placeholder="请输入名称">
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            ...
            <select name="seriesId" lay-verify="required" lay-search>
                ...
                <#list series as r>
                    <option value="${r.id}">${r.name}-${r.nameEn}</option>
                </#list>
            </select>
        </div>
    </div>
...
<script>
    layui.use(['form', 'jquery', 'layer'], function () {
        ...
        form.on("submit(addProductInfo)", function (data) {
            ...
            $.post("${base}/productInfo/add", data.field, function (res) {
                layer.close(loadIndex);
                if (res.success) {
                    parent.layer.msg("展示产品添加成功！", {time: 1000}, function () {
