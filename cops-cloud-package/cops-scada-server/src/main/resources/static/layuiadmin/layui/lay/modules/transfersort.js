...
layui.define(['laytpl', 'form'], function(exports) {
    ...
    var $ = layui.$,
        ...
        TPL_BOX = function(obj) {
            obj = obj || {};
            return ['<div class="layui-transfer-box" data-index="' + obj.index + '">', '<div class="layui-transfer-header">',
                '<input type="checkbox" name="' + obj.checkAllName +
                '" lay-filter="layTransferCheckbox" lay-type="all" lay-skin="primary" title="{{ d.data.title[' + obj.index +
                '] || \'list' + (obj.index + 1) + '\' }}">', '</div>', '{{# if(d.data.showSearch){ }}',
        ...
        TPL_MAIN = ['<div class="layui-transfer layui-form layui-border-box" lay-filter="LAY-transfer-{{ d.index }}">',
            ...
            '<i class="layui-icon layui-icon-up"></i>', '</button>',
            '<button type="button" class="layui-btn layui-btn-sm layui-btn-primary layui-btn-disabled" data-index="0">',
            '<i class="layui-icon layui-icon-next"></i>', '</button>',
            '<button type="button" class="layui-btn layui-btn-sm layui-btn-primary layui-btn-disabled" data-index="1">',
            '<i class="layui-icon layui-icon-prev"></i>', '</button>',
            '<button type="button" class="layui-btn layui-btn-sm layui-btn-primary layui-btn-disabled" data-index="2">',
            '<i class="layui-icon layui-icon-down"></i>', '</button>', '</div>', TPL_BOX({
                index: 1,
    ...
    Class.prototype.renderData = function() {
        ...
        that.parseData(function(item) {
            ...
            var _index = item.selected ? 1 : 0,
                listElem = ['<li>', '<input type="checkbox" name="' + arr[_index].checkName +
                '" lay-skin="primary" lay-filter="layTransferCheckbox" title="' + item.title + '"' + (item.disabled ?
                    ' disabled' : '') + (item.checked ? ' checked' : '') + ' value="' + item.value + '">', '</li>'
                ].join('');
        ...
        if (arr[1].views.length > 0) {
            let lst = new Array();
            for (let v of options.value) {
                for(let view of arr[1].views){
                    if($(view).children().val() == v){
                        lst.push(view);
                    }
                }
            }
            that.layData.eq(1).html(lst.join(''));
        }
        that.renderCheckBtn();
    }
    ...
    Class.prototype.renderCheckBtn = function(obj) {
        ...
        that.layBox.each(function(_index) {
            ...
            if (_index === 0) {
                that.layBtn.eq(1)[haveChecked ? 'removeClass' : 'addClass'](DISABLED); //对应的按钮状态
            } else {
                that.layBtn.eq(0)[haveChecked ? 'removeClass' : 'addClass'](DISABLED); //对应的按钮状态
                that.layBtn.eq(2)[haveChecked ? 'removeClass' : 'addClass'](DISABLED); //对应的按钮状态
                that.layBtn.eq(3)[haveChecked ? 'removeClass' : 'addClass'](DISABLED); //对应的按钮状态
            }
    ...
    Class.prototype.events = function() {
        ...
        that.layBtn.on('click', function() {
            ...
            if (_index === -1) {
                // 上移
                //找到选中的li
                var index = $(".layui-transfer ul").eq(1).find('.layui-form-checked').parent();
                //console.log(index);
                ...
                for (var i = index.length - 1; i >= 0; i--) {
                    if (index.eq(i).index() != ($(".layui-transfer ul").eq(1).find("li").length - 1)) {
                        index.eq(i).next().after(index.eq(i));
                    }
                }

            } else {
                that.layBox.eq(box_index).each(function(box_index) {
                    var othis = $(this),
                        thisDataElem = othis.find('.' + ELEM_DATA);

                    thisDataElem.children('li').each(function(li_index) {
                        var thisList = $(this),
