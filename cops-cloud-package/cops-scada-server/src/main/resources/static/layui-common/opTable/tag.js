	...
	var call = {
		...
		add: function(e, tagElem, options) {
				var filter = tagElem.attr('lay-filter'),
					...
					newTag = '<button lay-id="' + (options.id || '') + '"' + (options.attr ? ' lay-attr="' + options.attr + '"' :
						'') + ' type="button" class="' + TAG_ITEM + '">' + (options.text || 'unnaming') + '</button>';
		...
		input: function(e, othis) {
				...
				//标签输入框
				var inpatNewTag = $('<div class="' + INPUT_NEW_TAG +
